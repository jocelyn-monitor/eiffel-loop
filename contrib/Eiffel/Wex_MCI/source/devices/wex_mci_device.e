...
feature {NONE} -- Implementation
	...
	delete_device (parms: WEX_MCI_WAVE_DELETE_PARMS; delete_flags: INTEGER) is
			-- Start playing on a device.
		require
			opened: opened
			parms_not_void: parms /= Void
			parms_exists: parms.exists
		do
			send_command (Mci_play, delete_flags, parms)
		end
...
--				flags + command_flags, parms.to_integer)
			...
			error := cwin_mci_send_command_result (
				item.to_integer_32, command, flags + command_flags, parms.item.to_integer_32
			)
		ensure
...
feature {NONE} -- Removal
	...
	destroy_item is
		...
		do
			if opened then
--				WEXCHANGE
				cwin_mci_send_command (item.to_integer_32, Mci_close, Mci_wait, 0)
			end
		end

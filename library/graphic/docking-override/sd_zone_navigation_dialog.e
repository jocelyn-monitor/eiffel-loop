...
feature {NONE} -- Copied from Eiffel Build project GB_TIP_OF_THE_DAY_DIALOG
	...
	set_text (tip: STRING)
		...
		local
			...
			l_output, l_colon_slash: STRING
			l_maximum_string_width, l_start_pos: INTEGER
			l_all_space_indexes: ARRAYED_LIST [INTEGER]
			l_separator, l_dir_separator: CHARACTER
		do
			...
			l_dir_separator := Operating_environment.directory_separator
			l_colon_slash := ":"
			l_colon_slash.append_character (l_dir_separator)
			-- if it looks like a path or url
			if tip.index_of (l_dir_separator, 1) = 1 or else tip.has_substring (l_colon_slash) then
				l_separator := l_dir_separator
			else
				l_separator := ' '
			end
			...
			loop
				if l_modified_tip.item (l_counter) = l_separator then
			...
			loop
				...
				if l_lines.index < l_lines.count then
					l_output.append_character (l_separator)

note
	description: "[
		...
		Adapted from ISE Eiffel2Java JNI_Environment class.
		Holds information abouy JNI environment. Potentially many JNI environments can exists at once,
...
feature {JAVA_OBJECT_OR_CLASS} -- Java reference management

	increment_object_ref_count (java_object_id: POINTER)
			--
		do
			log.enter ("increment_object_ref_count")
	...
	decrement_object_ref_count (java_object_id: POINTER)
			--
		do
			log.enter ("decrement_object_ref_count")
			change_object_ref_count_by (java_object_id, -1)
			log.exit
		end
	...
	change_object_ref_count_by (java_object_id: POINTER; delta: INTEGER)
		...
		do
			...
			log.put_integer_field ("count", count.item)
			log.put_new_line
		end
...
feature -- Exception mechanism
	...
	throw_java_exception (jthrowable: OBSOLETE_JAVA_OBJECT)
...
feature {JAVA_CLASS, OBSOLETE_JAVA_OBJECT, JAVA_ARGS, JAVA_ARRAY}

	java_object_table: HASH_TABLE [OBSOLETE_JAVA_OBJECT, POINTER]
